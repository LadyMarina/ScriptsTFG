...
public class Noise : MonoBehaviour
{
    ...
    public static float[,] GenerateVoronoiNoise(int mapWidth, int mapHeight, int seed, int biomesGrid, int biomesNum, float noiseMult, float noiseDist)
    {
        float[,] biomesMap = new float[mapWidth, mapHeight];

        System.Random randomNumber = new System.Random(seed);
        SeedRandom.SetSeed(seed);

        int xS = randomNumber.Next(10, 20);
        int yS = randomNumber.Next(10, 20);
            ...
            for (int y = 0; y < mapHeight; y++)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    for (int j = 0; j < 4; j++)
                    {
                        ...
                        int dist = ((gridX + i) * biomesGrid + biomeX - x) * ((gridX + i) * biomesGrid + biomeX - x) + ((gridY + j) * biomesGrid + biomeY - y) * ((gridY + j) * biomesGrid + biomeY - y);

                        dist += (int)(Mathf.PerlinNoise(noiseDist * ((gridX + i) * biomesGrid + biomeX - x + offsetX) / 100f, noiseDist * ((gridY + j) * biomesGrid + biomeY - y + offsetY) / 100f) * noiseMult);
